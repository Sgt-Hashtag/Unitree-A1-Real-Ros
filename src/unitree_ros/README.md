...
# Dependencies
* [ROS](https://www.ros.org/) Melodic or ROS Noetic
...
If you face a dependency problem, you can just run `catkin_build` again.
...
```
roslaunch a1_description laikago_rviz.launch
```
...
```
Where the `rname` means robot name, which can be `aliengo`, `a1` or `go1`. The `wname` means world name, which can be `earth`, `space` or `stairs`. And the default value of `rname` is `laikago`, while the default value of `wname` is `earth`. In Gazebo, the robot should be lying on the ground with joints not activated.
...
```
rosrun unitree_controller unitree_move_noetic
```
